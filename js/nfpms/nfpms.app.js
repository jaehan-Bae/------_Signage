...
(function () {
  ...
  // lineName: UPPER/LOWER 이후만 흰색 처리
  const ZONES = ["UPPER", "LOWER"];

  function setLineName(id, rawText) {
    const el = document.getElementById(id);
    if (!el) return;
    ...
    const zone = ZONES.find((z) => upper.includes(z));
    if (!zone) {
      // 존 키워드 없으면 전체 텍스트
      el.textContent = text;
      return;
    ...
    const idx = upper.indexOf(zone);
    const before = text.slice(0, idx);
    const after = text.slice(idx);

    el.innerHTML = `
      <span class="line-plant">${escapeHtml(before)}</span>
      <span class="line-zone">${escapeHtml(after)}</span>
    `;
  }

  // 플랜트 dataset 세팅 (CSS 분기 트리거)
  // - line 객체에서 plant가 있으면 우선 사용
  // - 없으면 lineName / projectName / key에서 추론
  function resolvePlantKey(line) {
    if (!line) return "";

    if (line.plant) return String(line.plant).toLowerCase();

    const a = String(line.lineName || "").toLowerCase();
    const b = String(line.projectName || "").toLowerCase();
    const c = String(line.key || "").toLowerCase();

    const hay = `${a} ${b} ${c}`;

    if (hay.includes("cernavoda")) return "cernavoda";
    if (hay.includes("pickering")) return "pickering";
    return "";
  }

  function applyPlantDataset(line) {
    const wrap = document.querySelector(".nfpms-wrap");
    if (!wrap) return;

    const plantKey = resolvePlantKey(line);
    if (plantKey) wrap.dataset.plant = plantKey;
    else wrap.removeAttribute("data-plant"); // delete 보다 명확
  }
  ...
  function makeProcMetrics(line) {
    ...
    return line.processes.map((name, idx) => {
      const drift = Math.round((idx / Math.max(1, n - 1)) * 120);
  ...
  function renderLine(line) {
    // 플랜트 dataset 먼저 세팅 (이게 있어야 CSS가 갈라짐)
    applyPlantDataset(line);
    ...
    // 라인 타이틀 (UPPER/LOWER 이후 흰색)
    setLineName("lineName", line.lineName);
        ...
        card.innerHTML = `
          ...
          <div class="proc-bar">
            <div class="proc-track">
              <div class="proc-fill" style="width:${clamp(m.pct, 0, 100).toFixed(
                1
              )}%"></div>
            </div>
          ...
          <div class="proc-numbers">
            ...
            <div class="num-chip num-chip--remain ${
              isDelayed ? "num-chip--danger" : ""
            }"><span>DEL</span><b>${m.remain}</b></div>
          </div>
        `;
    ...
    const pct =
      line.totalTarget > 0
        ? Math.round((line.totalActual / line.totalTarget) * 100)
        : 0;
  ...
  window.addEventListener("load", () => {
    ...
    if (!lines.length) {
      console.warn(
        "[nfpms] window.NFPMS_LINES is empty or filtered out. Check nfpms.line.js + NFPMS_PAGE."
      );
      return;
    }
